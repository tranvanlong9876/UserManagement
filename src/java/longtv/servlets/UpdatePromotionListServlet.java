        ...
        try {
            ...
            if (account != null) {
                if (account.getRole().equals("Admin")) {
                    ...
                    int defaultRank = 5;
                    if(request.getParameter("dboPromotionRank") != null) {
                        defaultRank = Integer.parseInt(request.getParameter("dboPromotionRank"));
                    }
                    if (promotionList.getPromotionList().containsKey(userID)) {
                        promotionList.updateList(userID, defaultRank);
                        session.setAttribute("PROMOTIONDETAIL", promotionList);
                        url = SUCCESS;
                        request.setAttribute("STATUS_LIST", "Username: " + userID + "'s new rank has been updated to value: " + defaultRank + ".");
                    } else {
                        url = SUCCESS;
                        request.setAttribute("STATUS_LIST", "Can not find Username: " + userID + " to update.");
                    }
                }
            } else {
                url = NOTLOGIN;
            ...
            errorObj.setErrorServlet("Updating Promotion List");
            errorObj.setErrorDetail("Can not update promotion list right row, contact owner for more information!");
