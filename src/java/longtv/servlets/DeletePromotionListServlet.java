 ...
 * @author Admin
 */
@WebServlet(name = "DeletePromotionListServlet", urlPatterns = {"/DeletePromotionListServlet"})
public class DeletePromotionListServlet extends HttpServlet {

    private static final String ERROR = "error";
    private static final String SUCCESS = "promotionlist";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (account != null) {
                if (account.getRole().equals("Admin")) {
                    ...
                    if (promotionList != null) {
                        if (promotionList.getPromotionList().containsKey(userID)) {
                            promotionList.removeList(userID);
                            session.setAttribute("PROMOTIONDETAIL", promotionList);
                            url = SUCCESS;
                            request.setAttribute("STATUS_LIST", "Username: " + userID + " has been deleted from promotion list");
                        } else {
                            url = SUCCESS;
                            request.setAttribute("STATUS_LIST", "Can not find Username: " + userID + " from promotion list to remove.");
                        }
                    } else {
                        request.setAttribute("STATUS_LIST", "There's nothing left to delete.");
                    }
                } else {
                    url = INVALIDROLE;
                }
            ...
            errorObj.setErrorServlet("Deleting User From Promotion List");
